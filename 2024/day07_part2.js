$(document).ready(function(){
	...
	$(".submit_input").on("click",function(){
		...
		let validTotal = 0;
		for(let i=0;i<calibrationArray.length;i++){
			let maxPermutations = 3**(calibrationArray[i].length-2);
			for(let x=0;x<maxPermutations;x++){
				// Set up base 3 represntation of each operand's position for the current loop
				// 0 = addition, 1 = multiplication, 2 = concatination
				let operandsDecimal = (10**(calibrationArray[i].length-2)) + parseInt(x.toString(3));
				let operands = String(operandsDecimal).split('');
				operands = operands.map((x) => Number(x));
				operands.shift();
				// Perform operations based on positions for current loop
				let test = calibrationArray[i][1];
				let currentOperand = 0;
				calibrationArray[i].forEach(function(element,index){
					if(index == 0 || index == 1) return;
					else{
						if(operands[currentOperand] == 0) test += element;
						if(operands[currentOperand] == 1) test *= element;
						if(operands[currentOperand] == 2) test = parseInt("" + test + element); // INSANELY SLOW!!!!!
					}
					currentOperand++;
				});
